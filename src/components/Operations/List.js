            ...
            <Table className={classes.table} aria-label="table">
                ...
                {table.map((row) => (
                    ...
                    <TableCell className={classNames(classes.cellName, classes.cell)} component="th" scope="row">
                        <div className="icons">
                            <div className="operation-icon"><div className={`arrow ${row.name.toLowerCase()}`} /></div>
                <span className="operation-name">{row.name}</span>
                            <div className="currency-icons">
                                {row.name === 'Withdraw' && <div className="icon btc-out"/>}
                                {row.name === 'Deposit' && <div className="icon btc-in"/>}
                                {row.name === 'Transfer' && <div className="icon btc–btc"/>}
                                {row.name === 'Conversion' && <div className="icon btc-usd"/>}
                            </div>
                        </div>
                    </TableCell>
                    <TableCell className={classNames(classes.cellSum, classes.cell)} align="left">{row.sum}</TableCell>
                    <TableCell className={classNames(classes.cellBalance, classes.cell)} align="right">{row.balance}&nbsp;<span className={classes.cellCurrency}>{row.currency}</span></TableCell>
